...
The headline feature is that **the stack survives unattended reboots**: after
install, the server reboots, no one logs in, and Open WebUI still comes up on
`:80`.
...
12. Registers Scheduled Task `OpenWebUI` with `LogonType=Password` (you are
    prompted for the Windows password here).
...
| --- | --- | --- |
| `-Mode` | `Auto` | `Install` / `Fix` / `Diag` / `Test` / `Nssm` / `Uninstall` / `Auto` |
...
1. Waits for the network to have a real IPv4 address (up to 60s).
2. Waits for the AppX execution alias to be ready
   (`%LocalAppData%\Microsoft\WindowsApps\foundry.exe`, up to 60s).
3. Refuses to call any direct `Program Files\WindowsApps\...\foundry.exe`
   path — those fail with `Access is denied`.
4. Sanity-checks `foundry --version` and verifies the output looks like a
   real version string (some PS / AppX combinations don't surface exit codes).
5. Runs `foundry service start` and waits for the HTTP endpoint to respond.
6. Loads the model with **3 retries** and **per-attempt verification via
   `/v1/models`** — covers the common cold-boot race where
   `service start` returns before the inference subsystem is ready.
7. Launches `open-webui serve --host 0.0.0.0 --port <Port>` in the foreground
...
- **Task `LastResult = 0x80041315`** — `SeBatchLogonRight` missing for the
  run-as user. Newer revisions of this script grant the right automatically;
  if it still fails, run `secpol.msc` →
...
same password storage in LSA Secret, same constraints.
...
debugged on (Windows Server 2022, build 20348, Foundry Local 0.8.119) but
**has not been tested on every Windows edition, every build, every Foundry
version, or every account configuration**. Treat it as a starting point, not
a battle-hardened deployment artifact.

Read the code before running it on anything you care about. Open issues and
PRs are welcome — especially with logs from `-Mode Diag` or
`-Mode Test` — and the diagnostic codepath is designed to make it easy to
